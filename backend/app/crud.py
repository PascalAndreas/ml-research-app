...
def list_papers(session: Session, *, tag: Optional[int] = None, query: Optional[str] = None) -> List[Paper]:
    statement = select(Paper)
    if tag is not None:
        statement = statement.join(PaperTagLink).where(PaperTagLink.tag_id == tag)
    if query:
        like = f"%{query}%"
        statement = statement.where(Paper.title.ilike(like) | Paper.authors.ilike(like))
